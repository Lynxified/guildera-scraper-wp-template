(function () {
    ...
    S.sortResults = function (results) {
        ...
        if (sortVal === 'oldest') {
            ...
            sorted.sort(function (a, b) {
                var ea = (a.like_count || 0) + (a.retweet_count || 0) * 2 + (a.reply_count || 0) * 1.5;
                var eb = (b.like_count || 0) + (b.retweet_count || 0) * 2 + (b.reply_count || 0) * 1.5;
                return eb - ea;
            });
        } else {
    ...
    S.renderResultsCards = function (results) {
        ...
        results.forEach(function (item) {
            ...
            var avatarUrl = avatarSource ? (avatarSource.indexOf('pbs.twimg.com') > -1 ? avatarSource.replace(/\?(.*)/, '') : avatarSource) : '';
            // Add standard size suffix if missing
            if (avatarUrl && avatarUrl.indexOf('_400x400.') === -1 && avatarUrl.indexOf('.jpg') > -1) {
                avatarUrl = avatarUrl.replace('.jpg', '_400x400.jpg');
            }
            var avatarHtml = avatarUrl
                ? '<img class="card-avatar-img" src="' + S.escAttr(avatarUrl) + '" alt="" loading="lazy" />'
            ...
            html += '<div class="result-card">' +
                ...
                '<span class="card-stat" title="Bookmarks">&#x1F516; ' + (item.bookmark_count || 0).toLocaleString() + '</span>' +
    ...
    S.renderResultsTable = function (results) {
        ...
        results.forEach(function (item) {
            ...
            var engRetweets = (item.retweet_count || 0) * 3;
            var engBookmarks = (item.bookmark_count || 0) * 2;
            var engReplies = (item.reply_count || 0);
            var engViews = (item.view_count || 0);
            var engagementScore = (engLikes + engBookmarks + engRetweets + (engViews > 0 ? 13.5 * engReplies / engViews : 0)) * 1000;
